...
namespace dsp
{
  ...
  static thread_local window_t* running_ctx = nullptr;
  ...
  window_t::window_t(short _vid_w, short _vid_h) : image(nullptr, _vid_w, _vid_h, 1)
  {
    ...
    // So we pass &image.p because it was literally setup to be nullptr from the start.
    if (!( sys->hdib = CreateDIBSection( sys->hdc, (BITMAPINFO*)&di, DIB_RGB_COLORS, reinterpret_cast<void**>(&image.p), NULL, 0) ))
    {
  ...
  {
    BitBlt(sys->hdc, 0, 0, image.width, image.height, sys->hdibdc, 0, 0, SRCCOPY);
  }
  ...
  window_t::~window_t()
  {
    ...
    image.p = nullptr; // This is the nastiest hack known to man, the reason we do this is because the data there was not allocated with new but rather with WINAPI's RtlAllocateHeap(), took a long time to debug this on fucking Linux, but I finally did. And this destructor is gonna call ~map_t with delete [] p; 
  }
}
