...
namespace wav
{
  ...
  struct speaker_t
  {
    ...
    void swap_file(audio_t& f) { file = f; rewind(); }
  };
  ...
  // Can be changed
  static constexpr unsigned SAMPLE_RATE = 48'000;
  ...
  // Automatically determines buffer_duration.
  void initialize();
  
  // Begins by retreiving the buffer, after this you should call all the play() on the speaker_t's you created.
  void begin_playback();
